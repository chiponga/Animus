...
> - Agente principal (Animus) rodando 24/7 no tmux
> - 8 especialistas Animus
> - Bot externo Python (audio bidirecional)
> - agent-manager.py via PM2 (porta 3600 + Caddy + Cloudflare tunnel)
> - (Opcional) automacao comercial Apollo personalizada
...
1. **agent-manager.py** (Python via PM2): gerencia jobs longos sem travar a Animus. Roda na porta 3600 com proxy reverso via Caddy. Acessivel em `https://AGENTE.dominio.com`.
2. **Suite de subagentes especializados**: atlas, aegis, helena, victor, sentinel, titan, apollo, oracle.
...
4. Hierarquia (Dono manda, Animus orquestra, Animus coordena, especialistas executam)
5. REGRA SUPREMA - PROTOCOLO DE CONVERSA 3 FASES (igual ao da Animus)
6. ARQUITETURA DE ORQUESTRADORA (Animus delega, nao executa)
7. Lista dos 8 especialistas Animus
8. Como responder no Telegram (outbox JSON)
9. Voice ON/OFF (quando usar audio)
10. Anti-patterns (sem travessoes, sem voz de IA)

Cria os 8 especialistas em `/opt/AGENTE/.claude/agents/`:
- `atlas.md` (principal software engineer)
- `aegis.md` (principal cybersecurity and pentest engineer)
- `helena.md` (UI/UX e frontend design)
- `victor.md` (principal copywriter)
- `sentinel.md` (QA e reliability)
- `titan.md` (DevOps e infraestrutura)
- `apollo.md` (SDR e growth)
- `oracle.md` (dados e inteligencia estrategica)
...
## ETAPA 8 - CONFIGURAR APOLLO SDR/GROWTH (OPCIONAL)

Pergunta ao aluno se ele quer ativar uma automacao comercial guiada pelo Apollo para responder DMs, qualificar leads e registrar oportunidades.
...
Atualiza contexto comercial do Apollo com a personalidade configurada (rapport + SPIN + agendamento).

Configura webhook (se aluno tem CRM/integracao):
```bash
# Endpoint no agent-manager: POST /webhook/insta-dm
# Recebe DM, salva no banco, dispara fluxo comercial para Apollo
```
...
**Logs ao vivo:**
```bash
tail -f /opt/AGENTE/logs/agent.log         # Animus Claude
tail -f /opt/AGENTE-bot/logs/bot.log       # Bot Python
pm2 logs agent-manager                      # agent-manager
journalctl -u AGENTE -f                     # systemd Animus
```

**Restart:**
```bash
systemctl restart AGENTE          # restart Animus
...
**Editar subagente:**
```bash
nano /opt/AGENTE/.claude/agents/atlas.md
# nao precisa restart
```
