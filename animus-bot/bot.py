...
BOT_DIR = Path('/Users/animusrodrigues/animus-bot')
INBOX = BOT_DIR / 'inbox'
...
TMUX_SESSION = env.get('TMUX_SESSION', 'animus')
TMUX_USER = env.get('TMUX_USER', 'animus')
...
# Debounce: aguarda N segundos sem nova msg antes de injetar pra Animus.
# Quando chega nova msg, reseta o timer. Permite o usuario mandar msgs quebradas
# em sequencia que sao agrupadas como contexto unico antes de chegar na Animus.
...
def flush_pending():
    ...
    notify_animus(msg_id, text, user)
...
def notify_animus(msg_id, text, user):
    try:
        ...
        log.info(f'Animus notificada msg_id={msg_id}')
    except Exception as e:
        log.error(f'notify_animus error: {e}')
...
if __name__ == '__main__':
    log.info('=== Animus Telegram Bot iniciando ===')
    try:
