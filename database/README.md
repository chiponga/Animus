# Database Schema — animus_memory

Schema completo do banco PostgreSQL usado pelo agente Animus (memoria vetorial, SDR, DMs, transcricoes).
...
createdb -h 127.0.0.1 -U postgres animus_memory

# Habilitar pgvector dentro do banco recem-criado
psql -h 127.0.0.1 -U postgres -d animus_memory -c "CREATE EXTENSION IF NOT EXISTS vector;"
```

### 3. Aplicar o schema

```bash
psql -h 127.0.0.1 -U postgres -d animus_memory -f schema.sql
```

### 4. Validar

```bash
psql -h 127.0.0.1 -U postgres -d animus_memory -c "\dt"
...
Sistema de agentes comerciais e de crescimento coordenado por Apollo.
...
```bash
ssh root@{{VPS_IP}} "PGPASSWORD=*** pg_dump -h 127.0.0.1 -U n8n -s --no-owner --no-privileges --no-comments animus_memory" > schema.sql
